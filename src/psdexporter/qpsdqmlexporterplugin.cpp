...
QPsdQmlExporterPlugin::QPsdQmlExporterPlugin(QObject *parent)
    : QPsdExporterPlugin(parent)
{
}

QPsdQmlExporterPlugin::~QPsdQmlExporterPlugin() = default;

void QPsdQmlExporterPlugin::onBeforeExport(const ExportConfig &) const {}
...
void QPsdQmlExporterPlugin::stripLayerEffects(Element &element)
{
...
bool QPsdQmlExporterPlugin::exportTo(const QPsdExporterTreeItemModel *model,  const QString &to, const ExportConfig &config) const
{
    ...
    m_usedControls = false;

    onBeforeExport(config);
...
bool QPsdQmlExporterPlugin::outputBase(const QModelIndex &index, Element *element, ImportData *imports, QRect rectBounds) const
{
...
bool QPsdQmlExporterPlugin::outputRect(const QRectF &rect, Element *element, bool skipEmpty) const
{
...
bool QPsdQmlExporterPlugin::outputPath(const QPainterPath &path, Element *element) const
{
...
bool QPsdQmlExporterPlugin::outputFolder(const QModelIndex &folderIndex, Element *element, ImportData *imports, ExportData *exports, QPsdBlend::Mode groupBlendMode) const
{
...
void QPsdQmlExporterPlugin::applyBlendModes(Element *element, ImportData *imports) const
{
...
bool QPsdQmlExporterPlugin::outputShape(const QModelIndex &shapeIndex, Element *element, ImportData *imports) const
{
    ...
    case QPsdAbstractLayerItem::PathInfo::Rectangle: {
        ...
        if (g) {
            ...
            if (needsShapePath) {
                ...
                if (pen.style() != Qt::NoPen && supportsRectangleBorder()) {
                    qreal dw = computeStrokeWidth(pen, unitScale);
...
bool QPsdQmlExporterPlugin::outputImage(const QModelIndex &imageIndex, Element *element, ImportData *imports) const
{
    ...
    if (border && border->isEnable() && supportsRectangleBorder()) {
        Element wrapper;
...
bool QPsdQmlExporterPlugin::traverseTree(const QModelIndex &index, Element *parent, ImportData *imports, ExportData *exports, std::optional<QPsdExporterTreeItemModel::ExportHint::Type> hintOverload, QPsdBlend::Mode groupBlendMode) const
{
    const QPsdAbstractLayerItem *item = model()->layerItem(index);
    ...
    case QPsdExporterTreeItemModel::ExportHint::Component: {
        ...
        // Component files have their own coordinate space; x/y belong on the
        // instance side only. anchors.fill: parent likewise belongs on the
        // instance — putting it on the component root forces every instance to
        // fill its parent regardless of x/y/width/height set on the instance
        // side, breaking the reusable-component contract. The instance still
        // receives anchors.fill from outputBase below when its source rect is
        // empty.
...
bool QPsdQmlExporterPlugin::saveTo(const QString &baseName, Element *element, const ImportData &imports, const ExportData &exports) const
{
    if (imports.contains("QtQuick.Controls"_L1))
        m_usedControls = true;
